...
SCALP_MIN_CANDIDATE_SCORE = 52
SCALP_MIN_AI_SCORE = 62
SCALP_MIN_QUALITY_SCORE = 58
SCALP_MIN_ADX = 12
SCALP_STRONG_ADX = 18
SCALP_MIN_M1_SCORE = 48
SCALP_MIN_M5_SCORE = 48
SCALP_MIN_MICRO_SCORE = 45
SCALP_MIN_MONEY_SCORE = 50
...
# ============================================================
def calculate_strategy_fusion(signal, scalp, sp2l, bb_stoch, micro,
                              money_structure, structure, liquidity, fvg,
                              displacement, lux_ob=None):
    """
    Combines the already-existing engines into one transparent confluence
    score. It is intentionally a SOFT layer; individual modules keep their
    own logic and no single indicator can force an entry.
    """
    try:
        ...
        def add_if(name, condition, points):
            nonlocal score
            if condition:
                score += points
                aligned.append(name)
        ...
        # Execution trigger.
        scalp_dir = scalp.get("direction", 0)
        if scalp_dir == direction:
            add_if("M1/M5 trigger", True, 14)
        elif scalp_dir == -direction:
            score -= 10
            conflicts.append("M1/M5 trigger")

        # SP2L.
        sp2l_dir = sp2l.get("direction", 0)
        if sp2l_dir == direction:
            add_if("SP2L", True, 7)
        elif sp2l_dir == -direction:
            score -= 5
            conflicts.append("SP2L")

        # BB + Stochastic.
        bb_dir = bb_stoch.get("direction", 0)
        if bb_dir == direction:
            add_if("BB+Stoch", True, 7)
        elif bb_dir == -direction:
            score -= 4
            conflicts.append("BB+Stoch")

        # Microstructure / Money Structure.
        if microstructure_confirms(signal, micro):
            add_if("Microstructure", True, 8)
        elif micro.get("direction") not in ("NEUTRAL", None):
            score -= 5
            conflicts.append("Microstructure")

        money_dir = int(money_structure.get("direction", 0) or 0)
        if money_dir == direction:
            add_if("Money Structure", True, 8)
        elif money_dir == -direction:
            score -= 5
            conflicts.append("Money Structure")

        # Smart-money structure.
        structure_ok = (
            structure.get("bullish_bos") or structure.get("bullish_choch")
            if direction == 1 else
            structure.get("bearish_bos") or structure.get("bearish_choch")
        )
        liquidity_ok = (
            liquidity.get("bullish") if direction == 1 else liquidity.get("bearish")
        )
        fvg_ok = (
            fvg.get("bullish") if direction == 1 else fvg.get("bearish")
        )
        displacement_ok = (
            displacement.get("bullish") if direction == 1 else displacement.get("bearish")
        )

        add_if("BOS/CHoCH", structure_ok, 8)
        add_if("Liquidity", liquidity_ok, 7)
        add_if("FVG", fvg_ok, 5)
        add_if("Displacement", displacement_ok, 5)

        # A strong opposite smart-money signal is treated as contradiction.
        opposite_structure = (
            structure.get("bearish_bos") or structure.get("bearish_choch")
            if direction == 1 else
            structure.get("bullish_bos") or structure.get("bullish_choch")
        ...
        if opposite_structure:
            score -= FUSION_CONTRADICTION_PENALTY
            conflicts.append("Opposite structure")

        if lux_ob:
            lo_dir=int(lux_ob.get("direction",0) or 0); lo_score=int(lux_ob.get("score",50) or 50)
            if lo_dir==direction: add_if("Lux+Volumetric OB", True, 12 if lo_score>=70 else 7)
            elif lo_dir==-direction: score-=5; conflicts.append("Lux direction")
        score = int(max(0, min(100, round(score))))
        return {
            "score": score,
            "passed": score >= FUSION_MIN_SCORE,
            "aligned": aligned,
            "conflicts": conflicts,
        }
    except Exception as e:
        print(f"Strategy fusion error: {e}")
        return {
            "score": 50,
            "passed": False,
            "aligned": [],
            "conflicts": ["fusion error"],
        }


# ============================================================
# PREMIUM LUX + VOLUMETRIC ORDER BLOCK ENGINE
# ============================================================
def _lux_supertrend(df, atr_period=11, factor=5.5):
    try:
        h,l,c=df["high"].astype(float),df["low"].astype(float),df["close"].astype(float)
        atr=ta.volatility.average_true_range(h,l,c,atr_period); mid=(h+l)/2
        up=mid+factor*atr; dn=mid-factor*atr; trend=1; pu=pl=None; dirs=[]
        for i in range(len(c)):
            u=float(up.iloc[i]) if pd.notna(up.iloc[i]) else None; d=float(dn.iloc[i]) if pd.notna(dn.iloc[i]) else None
            if u is None or d is None: dirs.append(trend); continue
            cp=float(c.iloc[i-1]) if i and pd.notna(c.iloc[i-1]) else None; cn=float(c.iloc[i])
            if pu is not None and cp is not None and cp<=pu: u=min(u,pu)
            if pl is not None and cp is not None and cp>=pl: d=max(d,pl)
            if pu is not None and cn>pu: trend=1
            elif pl is not None and cn<pl: trend=-1
            dirs.append(trend); pu,pl=u,d
        return int(dirs[-2] if len(dirs)>=2 else dirs[-1]), atr
    except Exception: return 0,pd.Series(dtype=float)

def _lux_range_filter(close,period=14,mult=2.618,smooth_period=27):
    try:
        c=close.astype(float); rng=c.diff().abs().rolling(period).mean().ewm(span=smooth_period,adjust=False).mean()*mult
        filt=pd.Series(index=c.index,dtype=float); last=None
        for i in range(len(c)):
            ci=float(c.iloc[i]); r=float(rng.iloc[i]) if pd.notna(rng.iloc[i]) else None
            if r is None or r<=0: filt.iloc[i]=ci if last is None else last; continue
            if last is None: last=ci
            elif ci>last: last=max(last,ci-r)
            else: last=min(last,ci+r)
            filt.iloc[i]=last
        i=-2 if len(c)>=2 else -1
        if pd.isna(filt.iloc[i]): return 0,None
        d=1 if c.iloc[i]>filt.iloc[i] else -1 if c.iloc[i]<filt.iloc[i] else 0
        return d,float(filt.iloc[i])
    except Exception: return 0,None

def _lux_tbo(close,fast=5,medium=13,slow=34):
    try:
        c=close.astype(float); ef=ta.trend.ema_indicator(c,fast); em=ta.trend.ema_indicator(c,medium); es=ta.trend.ema_indicator(c,slow); i=-2 if len(c)>=2 else -1
        if any(pd.isna(x.iloc[i]) for x in (ef,em,es)): return 0,False,False
        buy=float(ef.iloc[i])>float(em.iloc[i])>float(es.iloc[i]); sell=float(ef.iloc[i])<float(em.iloc[i])<float(es.iloc[i])
        return (1 if buy else -1 if sell else 0),buy,sell
    except Exception: return 0,False,False

def _lux_smart_trail(df,atr_period=13,factor=4.0):
    try:
        h,l,c=df["high"].astype(float),df["low"].astype(float),df["close"].astype(float); atr=ta.volatility.average_true_range(h,l,c,atr_period); i=-2 if len(c)>=2 else -1
        if pd.isna(atr.iloc[i]): return 0,None
        ema=ta.trend.ema_indicator(c,atr_period); d=1 if float(c.iloc[i])>float(ema.iloc[i]) else -1 if float(c.iloc[i])<float(ema.iloc[i]) else 0
        return d,float(c.iloc[i]-factor*atr.iloc[i])
    except Exception: return 0,None

def _lux_reversal_context(close):
    try:
        r=ta.momentum.rsi(close.astype(float),14); x=float(r.iloc[-2] if len(r)>=2 else r.iloc[-1]); return {"rsi":x,"oversold":x<=25,"overbought":x>=75}
    except Exception: return {"rsi":50.0,"oversold":False,"overbought":False}

def detect_volumetric_order_block(open_price,close,high,low,volume,direction):
    neutral={"valid":False,"direction":0,"high":None,"low":None,"mid":None,"volume":0.0,"activity":0.0,"age":None}
    try:
        n=len(close); end=n-2
        if n<20: return neutral
        vol=volume.astype(float).fillna(0); start=max(2,end-OB_MAX_AGE)
        for i in range(end-2,start-1,-1):
            v=float(vol.iloc[i]); left=vol.iloc[i-2:i]; right=vol.iloc[i+1:i+3]
            if v<=0 or v<float(max(left.max(),right.max())): continue
            o,c,h,l=map(float,(open_price.iloc[i],close.iloc[i],high.iloc[i],low.iloc[i])); obdir=1 if c<o else -1 if c>o else 0
            if obdir!=direction: continue
            now=float(close.iloc[end])
            if (now<l if direction==1 else now>h): continue
            av=float(vol.iloc[max(0,end-OB_VOLUME_LOOKBACK):end].mean()); activity=v/av*100 if av>0 else 50
            return {"valid":True,"direction":direction,"high":h,"low":l,"mid":(h+l)/2,"volume":v,"activity":min(200,activity),"age":end-i}
        return neutral
    except Exception: return neutral

def analyze_lux_ob_fusion(m1,m5,direction):
    neutral={"score":50,"direction":0,"buy":False,"sell":False,"lux_score":50,"ob_score":50,"aligned":[],"conflicts":[],"ob":{},"smart_trail":0,"range_filter":0,"tbo":0,"supertrend":0,"reversal_rsi":50.0}
    try:
        if not LUX_OB_FUSION_ENABLED or m1 is None or m5 is None: return neutral
        vals=[]
        for df in (m5,m1):
            st,_=_lux_supertrend(df,LUX_SUPERTREND_ATR,LUX_SUPERTREND_FACTOR); rf,_=_lux_range_filter(df["close"],LUX_RANGE_PERIOD,LUX_RANGE_MULT,LUX_RANGE_SMOOTH); tbo,_,_=_lux_tbo(df["close"],LUX_TBO_FAST,LUX_TBO_MED,LUX_TBO_SLOW); tr,_=_lux_smart_trail(df,LUX_SMART_TRAIL_ATR,LUX_SMART_TRAIL_FACTOR); vals.append((st,rf,tbo,tr))
        rev=_lux_reversal_context(m1["close"]); lux=50; aligned=[]; conflicts=[]
        for label,vals4 in zip(("M5","M1"),vals):
            for n,v in zip(("Supertrend","Range Filter","TBO","Smart Trail"),vals4):
                if v==direction: lux+=6; aligned.append(f"{label} {n}")
                elif v==-direction: lux-=4; conflicts.append(f"{label} {n}")
        if direction==1 and rev["oversold"]: lux+=5; aligned.append("M1 reversal")
        if direction==-1 and rev["overbought"]: lux+=5; aligned.append("M1 reversal")
        if direction==1 and rev["overbought"]: lux-=8; conflicts.append("M1 overbought")
        if direction==-1 and rev["oversold"]: lux-=8; conflicts.append("M1 oversold")
        lux=max(0,min(100,lux)); ob=detect_volumetric_order_block(m1["open"],m1["close"],m1["high"],m1["low"],m1["volume"],direction)
        if not ob["valid"]: ob=detect_volumetric_order_block(m5["open"],m5["close"],m5["high"],m5["low"],m5["volume"],direction)
        obs=50
        if ob["valid"]: obs=95 if ob["activity"]>=130 else 88 if ob["activity"]>=105 else 80; aligned.append("Volumetric OB")
        score=int(max(0,min(100,round(lux*.65+obs*.35))))
        return {"score":score,"direction":direction,"buy":direction==1 and score>=LUX_FUSION_MIN_SCORE,"sell":direction==-1 and score>=LUX_FUSION_MIN_SCORE,"lux_score":lux,"ob_score":obs,"aligned":aligned,"conflicts":conflicts,"ob":ob,"smart_trail":vals[0][3],"range_filter":vals[0][1],"tbo":vals[0][2],"supertrend":vals[0][0],"reversal_rsi":rev["rsi"]}
    except Exception as e: print(f"Lux/OB fusion error: {e}"); return neutral

# ============================================================
# BOLLINGER + STOCHASTIC SCALP CONFIRMATION
# ============================================================
# Added from the requested 5-minute scalp concept.
# This is a SOFT confirmation layer: it never creates a trade by itself.
BB_STOCH_ENABLED = True
BB_PERIOD = 20
BB_STD = 2.0
STOCH_WINDOW = 14
STOCH_SMOOTH = 3
BB_STOCH_BONUS = 6
BB_STOCH_MIN_SCORE = 55

# ============================================================
# STRATEGY FUSION / ANTI-OVERFILTERING
# ============================================================
# M5 remains the execution-context anchor. M1 may temporarily pull
# against M5 during a retracement; this is not automatically a rejection.
FUSION_ENABLED = True
FUSION_MIN_SCORE = 55
FUSION_M5_WEIGHT = 0.60
FUSION_M1_WEIGHT = 0.40
FUSION_CONTRADICTION_PENALTY = 6

# ============================================================
# PREMIUM LUX + VOLUMETRIC ORDER BLOCK FUSION
# ============================================================
# Soft translations of the supplied Pine indicators.
LUX_OB_FUSION_ENABLED = True
LUX_FUSION_BONUS = 12
LUX_FUSION_MIN_SCORE = 55
LUX_SUPERTREND_ATR = 11
LUX_SUPERTREND_FACTOR = 5.5
LUX_RANGE_PERIOD = 14
LUX_RANGE_MULT = 2.618
LUX_RANGE_SMOOTH = 27
LUX_SMART_TRAIL_ATR = 13
LUX_SMART_TRAIL_FACTOR = 4.0
LUX_TBO_FAST = 5
LUX_TBO_MED = 13
LUX_TBO_SLOW = 34
OB_VOLUME_LOOKBACK = 20
OB_PIVOT_LEFT = 2
OB_PIVOT_RIGHT = 2
OB_MAX_AGE = 80
...
# BOLLINGER + STOCHASTIC SCALP CONFIRMATION
# ============================================================
def analyze_bollinger_stochastic(m1, m5):
    """
    Soft M1/M5 confirmation using Bollinger Bands + Stochastic.

    The image/source concept describes short-term opportunities from
    Bollinger Bands + Stochastic. It does not define a guaranteed entry,
    so this module scores context instead of forcing a trade.
    """
    neutral = {
        "buy": False, "sell": False, "direction": 0, "score": 50,
        "m1_score": 50, "m5_score": 50,
        "bb_m1": "NEUTRAL", "bb_m5": "NEUTRAL",
        "stoch_m1": 50.0, "stoch_m5": 50.0,
        "reason": "BB/Stochastic unavailable",
    }

    try:
        if not BB_STOCH_ENABLED or m1 is None or m5 is None:
            return neutral
        ...
        def frame_signal(df):
            ...
            if len(c) < max(BB_PERIOD + 5, STOCH_WINDOW + STOCH_SMOOTH + 5):
                return 50, 50, "NEUTRAL", 50.0, ""
            ...
            upper = float(bb.bollinger_hband().iloc[i])
            lower = float(bb.bollinger_lband().iloc[i])
            middle = float(bb.bollinger_mavg().iloc[i])

            # Position inside the Bollinger envelope.
            band_width = max(upper - lower, 1e-12)
            bb_pos = (close_now - lower) / band_width
            ...
            k = float(st.stoch().iloc[i])
            d = float(st.stoch_signal().iloc[i])

            buy = 0
            sell = 0
            reasons = []

            # Mean-reversion context: lower band + rising stochastic.
            if close_now <= lower * 1.001:
                buy += 30
                reasons.append("near lower BB")
            elif bb_pos <= 0.30:
                buy += 15

            if close_now >= upper * 0.999:
                sell += 30
            ...
            # Oversold/overbought gives extra context, but is not an entry.
            if k <= 20:
                buy += 20
            elif k >= 80:
                sell += 20

            # Avoid blindly buying an already overbought market or selling
            ...
            if buy > sell:
                ...
                direction = 0
                score = max(buy, sell, 50)

            bb_state = (
                "LOWER" if bb_pos <= 0.30
                else "UPPER" if bb_pos >= 0.70
                else "MIDDLE"
            )
            return score, direction, bb_state, k, ", ".join(reasons)

        m1_score, m1_dir, m1_bb, m1_k, m1_reason = frame_signal(m1)
        m5_score, m5_dir, m5_bb, m5_k, m5_reason = frame_signal(m5)

        buy_score = 0
        sell_score = 0

        # M5 is the primary context; M1 is the trigger confirmation.
        if m5_dir == 1:
            buy_score += int(m5_score * 0.60)
        elif m5_dir == -1:
            sell_score += int(m5_score * 0.60)

        if m1_dir == 1:
            buy_score += int(m1_score * 0.40)
        elif m1_dir == -1:
            sell_score += int(m1_score * 0.40)

        # If both timeframes agree, give a small alignment boost.
        if m1_dir == 1 and m5_dir == 1:
            buy_score += 10
        elif m1_dir == -1 and m5_dir == -1:
            sell_score += 10

        buy_score = max(0, min(100, buy_score))
        sell_score = max(0, min(100, sell_score))

        if buy_score > sell_score and buy_score >= BB_STOCH_MIN_SCORE:
            direction = 1
            score = buy_score
        elif sell_score > buy_score and sell_score >= BB_STOCH_MIN_SCORE:
            direction = -1
            score = sell_score
        else:
            direction = 0
            score = max(50, buy_score, sell_score)

        reasons = [x for x in (m1_reason, m5_reason) if x]
        return {
            "buy": direction == 1,
            "sell": direction == -1,
            "direction": direction,
            "score": int(max(0, min(100, score))),
            "m1_score": int(m1_score),
            "m5_score": int(m5_score),
            "bb_m1": m1_bb,
            "bb_m5": m5_bb,
            "stoch_m1": round(m1_k, 2),
            "stoch_m5": round(m5_k, 2),
            "reason": "; ".join(reasons) if reasons else "No BB/Stochastic alignment",
        }

    except Exception as e:
        print(f"BB/Stochastic error: {e}")
        return neutral
...
# ============================================================
# PREVIOUS-DAY LIQUIDITY HUNT / SINGLE-CANDLE STRATEGY
# ============================================================
# 1D: previous day's High, Low and 50% midpoint.
# 15M BUY: context below PD50 -> sweep PDL -> close back above PDL.
# 15M SELL: context above PD50 -> sweep PDH -> close back below PDH.
# PD50 is a no-trade zone when price is ranging around the midpoint.
# Integrated as a SOFT confirmation for the existing M1/M5 scalp engine.
PREV_DAY_LIQUIDITY_ENABLED = True
PREV_DAY_LIQUIDITY_HARD_FILTER = False
PREV_DAY_MID_NO_TRADE_ATR = 0.20
PREV_DAY_LIQUIDITY_BONUS = 10
PREV_DAY_LIQUIDITY_PENALTY = 0
PREV_DAY_LIQUIDITY_MIN_SCORE = 60
...
# ============================================================
# PREVIOUS-DAY LIQUIDITY HUNT ENGINE
# ============================================================
def analyze_previous_day_liquidity(d1, m15, current_price=None):
    """Previous-day liquidity hunt using 1D PDH/PDL/50% and 15M sweep/reclaim."""
    neutral = {
        "available": False, "buy": False, "sell": False, "direction": 0,
        "score": 0, "status": "UNAVAILABLE", "reason": "Previous-day liquidity data unavailable",
        "pdh": None, "pdl": None, "pd50": None, "current_price": current_price,
        "sweep_high": False, "sweep_low": False, "midpoint_zone": False,
        "tp1": None, "tp2": None
    }
    try:
        if d1 is None or m15 is None:
            return neutral
        ddf = pd.DataFrame({"open": d1["open"], "high": d1["high"], "low": d1["low"], "close": d1["close"]}).dropna()
        if len(ddf) < 3:
            return neutral
        didx = pd.DatetimeIndex(ddf.index)
        if didx.tz is None:
            didx = didx.tz_localize("UTC")
        ddf.index = didx.tz_convert(ZoneInfo(OPENING_RANGE_TIMEZONE))
        ddf = ddf.sort_index()
        today_local = datetime.datetime.now(ZoneInfo(OPENING_RANGE_TIMEZONE)).date()
        completed = ddf[ddf.index.date < today_local]
        if completed.empty:
            return neutral
        prev = completed.iloc[-1]
        pdh, pdl = float(prev["high"]), float(prev["low"])
        if not (is_valid_number(pdh) and is_valid_number(pdl) and pdh > pdl):
            return neutral
        pd50 = (pdh + pdl) / 2.0

        mdf = pd.DataFrame({"open": m15["open"], "high": m15["high"], "low": m15["low"], "close": m15["close"]}).dropna()
        if len(mdf) < 20:
            return neutral
        midx = pd.DatetimeIndex(mdf.index)
        if midx.tz is None:
            midx = midx.tz_localize("UTC")
        mdf.index = midx.tz_convert(ZoneInfo(OPENING_RANGE_TIMEZONE))
        mdf = mdf.sort_index()
        i = len(mdf) - 2
        if i < 1:
            return neutral
        ...
        atr15 = safe_float(ta.volatility.average_true_range(mdf["high"], mdf["low"], mdf["close"], 14), i) or 0.0
        midpoint_buffer = max(atr15 * PREV_DAY_MID_NO_TRADE_ATR, (pdh - pdl) * 0.02)
        midpoint_zone = abs(c - pd50) <= midpoint_buffer
        sweep_low = l <= pdl and c > pdl
        sweep_high = h >= pdh and c < pdh
        buy_setup = c < pd50 and sweep_low
        sell_setup = c > pd50 and sweep_high
        strong_buy = buy_setup and close_location >= 0.60 and (atr15 <= 0 or body >= atr15 * 0.20)
        strong_sell = sell_setup and close_location <= 0.40 and (atr15 <= 0 or body >= atr15 * 0.20)

        if midpoint_zone:
            status, reason, direction, buy, sell, score = "NO_TRADE_MIDPOINT", "Price is ranging near previous-day 50% midpoint", 0, False, False, 0
        elif strong_buy:
            status, reason, direction, buy, sell, score = "BUY_LIQUIDITY_HUNT", "PDL swept and 15M close returned above PDL below PD50", 1, True, False, 100
        elif strong_sell:
            status, reason, direction, buy, sell, score = "SELL_LIQUIDITY_HUNT", "PDH swept and 15M close returned below PDH above PD50", -1, False, True, 100
        elif buy_setup:
            status, reason, direction, buy, sell, score = "BUY_SWEEP_WEAK_RETURN", "PDL swept/reclaimed but 15M return is weak", 1, False, False, 60
        elif sell_setup:
            status, reason, direction, buy, sell, score = "SELL_SWEEP_WEAK_RETURN", "PDH swept/rejected but 15M return is weak", -1, False, False, 60
        else:
            status, reason, direction, buy, sell, score = "WAITING_SWEEP", "Waiting for previous-day high/low liquidity sweep", 0, False, False, 0

        neutral.update({"available": True, "buy": buy, "sell": sell, "direction": direction,
                        "score": score, "status": status, "reason": reason,
                        "pdh": pdh, "pdl": pdl, "pd50": pd50,
                        "current_price": c if current_price is None else current_price,
                        "sweep_high": sweep_high, "sweep_low": sweep_low,
                        "midpoint_zone": midpoint_zone,
                        "tp1": pd50 if direction else None,
                        "tp2": pdh if direction == 1 else pdl if direction == -1 else None,
                        "atr15": atr15, "midpoint_buffer": midpoint_buffer,
                        "candle_open": o, "candle_close": c, "close_location": close_location})
        return neutral
    except Exception as e:
        print(f"Previous-day liquidity error: {e}")
        return neutral
...
def master_quality_filter(
    ...
    scalp=None, micro_score=0, money_score=0, fusion_score=50
):
    ...
    if SCALP_SUB100_ENABLED:
        ...
        if (scalp.get("m1_score",0) < SCALP_MIN_M1_SCORE or scalp.get("m5_score",0) < SCALP_MIN_M5_SCORE) and fusion_score < 65:
            return False, "M1/M5 trigger confirmation too weak"
        if micro_score < SCALP_MIN_MICRO_SCORE and money_score < SCALP_MIN_MONEY_SCORE and fusion_score < 60:
            return False, "Need Microstructure or Money Structure confirmation"
        if not trend_aligned and fusion_score < 65:
            return False, "M1/M5 trend conflict"
        if not rsi_valid and fusion_score < 60:
...
def analyze_market(
    ...
    try:
        ...
        d1 = prepare_data(symbol, "1d") if symbol == "GC=F" and PREV_DAY_LIQUIDITY_ENABLED else None
        if symbol == "GC=F" and PREV_DAY_LIQUIDITY_ENABLED and d1 is None:
            print(f"{name}: Previous-day daily data unavailable - liquidity module neutral")
        ...
        # ====================================================
        # PREVIOUS-DAY LIQUIDITY HUNT
        # ====================================================
        prev_day_liquidity = (
            analyze_previous_day_liquidity(d1, m15, None)
            if symbol == "GC=F" and PREV_DAY_LIQUIDITY_ENABLED
            else {"available": False, "buy": False, "sell": False, "direction": 0,
                  "score": 0, "status": "DISABLED", "reason": "disabled",
                  "pdh": None, "pdl": None, "pd50": None, "midpoint_zone": False,
                  "tp1": None, "tp2": None}
        )
        print(f"{name}: PD Liquidity status={prev_day_liquidity.get('status')} PDH={prev_day_liquidity.get('pdh')} PD50={prev_day_liquidity.get('pd50')} PDL={prev_day_liquidity.get('pdl')} BUY={prev_day_liquidity.get('buy')} SELL={prev_day_liquidity.get('sell')}")
        if symbol == "GC=F" and PREV_DAY_LIQUIDITY_ENABLED and prev_day_liquidity.get("midpoint_zone"):
            print(f"{name}: Previous-day 50% NO-TRADE ZONE - no signal")
            return None
        ...
        bb_stoch = analyze_bollinger_stochastic(m1, m5) if SCALP_SUB100_ENABLED else {}
        ...
        print(f"{name}: BB/Stoch BUY={bb_stoch.get('buy',False)} SELL={bb_stoch.get('sell',False)} DIR={bb_stoch.get('direction',0)} SCORE={bb_stoch.get('score',50)} M1K={bb_stoch.get('stoch_m1',50)} M5K={bb_stoch.get('stoch_m5',50)}")
        ...
        if bb_stoch.get("buy"): buy_score += BB_STOCH_BONUS
        if bb_stoch.get("sell"): sell_score += BB_STOCH_BONUS
        ...
        # Previous-day liquidity hunt is a soft directional confirmation.
        if prev_day_liquidity.get("buy"):
            buy_score += PREV_DAY_LIQUIDITY_BONUS
        elif prev_day_liquidity.get("sell"):
            sell_score += PREV_DAY_LIQUIDITY_BONUS
        if PREV_DAY_LIQUIDITY_HARD_FILTER and prev_day_liquidity.get("direction", 0) == 0:
            print(f"{name}: Previous-day liquidity hard filter rejected - no fresh hunt")
            return None
        ...
        # Premium Lux + Volumetric OB is a soft confirmation layer.
        lux_ob = analyze_lux_ob_fusion(m1, m5, 1 if signal == "🟢 BUY" else -1) if LUX_OB_FUSION_ENABLED else {}
        print(f"{name}: Lux+OB SCORE={lux_ob.get('score',50)} LUX={lux_ob.get('lux_score',50)} OB={lux_ob.get('ob_score',50)} ST={lux_ob.get('supertrend',0)} RF={lux_ob.get('range_filter',0)} TBO={lux_ob.get('tbo',0)} OB_VALID={lux_ob.get('ob',{}).get('valid',False)}")

        # Strategy Fusion is evaluated only after a direction exists.
        fusion = calculate_strategy_fusion(
            signal, scalp, sp2l, bb_stoch, micro, money_structure,
            structure, liquidity, fvg, displacement, lux_ob
        ) if FUSION_ENABLED else {"score": 50, "passed": True, "aligned": [], "conflicts": []}

        if fusion["passed"]:
            preliminary = min(100, preliminary + int(round((fusion["score"] - 50) * 0.20)))
        elif fusion["score"] < 50:
            preliminary = max(0, preliminary - int(round((50 - fusion["score"]) * 0.15)))

        print(
            f"{name}: FUSION score={fusion['score']}/100 "
            f"aligned={len(fusion['aligned'])} "
        ...
        if signal == "🟢 BUY":
            # M5 is the primary scalp context. M1 is allowed to be a temporary
            # pullback when the rest of the setup has confluence.
            trend_aligned = (
                scalp.get("m5_trend") == 1 and
                (scalp.get("m1_trend") in (1, 0) or fusion["score"] >= 72)
            )
        else:
            trend_aligned = (
                scalp.get("m5_trend") == -1 and
        ...
        if SCALP_SUB100_ENABLED:
            ...
            if prev_day_liquidity.get("direction") == (1 if signal == "🟢 BUY" else -1):
                quality_score = min(100, quality_score + PREV_DAY_LIQUIDITY_BONUS)
            if bb_stoch.get("direction") == (1 if signal == "🟢 BUY" else -1):
                quality_score = min(100, quality_score + BB_STOCH_BONUS)
            if lux_ob.get("direction") == (1 if signal == "🟢 BUY" else -1) and lux_ob.get("score", 50) >= LUX_FUSION_MIN_SCORE:
                quality_score = min(100, quality_score + LUX_FUSION_BONUS)
        ...
        if SCALP_SUB100_ENABLED:
            ...
            final_ai_score = int(max(0, min(100, round(
                quality_score * 0.39 +
                scalp.get("score", 0) * 0.27 +
                micro.get("score", 50) * 0.14 +
                smart_score * 0.05 +
                sp2l.get("score", 0) * 0.03 +
                prev_day_liquidity.get("score", 0) * 0.03 +
                bb_stoch.get("score", 50) * 0.03 +
                fusion.get("score", 50) * 0.04 +
                lux_ob.get("score", 50) * 0.02
            ))))
        else:
        ...
        if SCALP_SUB100_ENABLED:
            ...
            if news_risk == "HIGH":
                ...
                return None
            if not trend_aligned and fusion.get("score", 0) < 65:
        ...
        passed, reason = master_quality_filter(
            ...
            fusion_score=int(fusion.get("score", 50))
        )
        ...
        bb_stoch_status = (
            "BUY" if bb_stoch.get("buy")
            else "SELL" if bb_stoch.get("sell")
            else "NEUTRAL"
        )
...
BB+Stoch: {bb_stoch_status} {bb_stoch.get("score", 50)}/100
Lux+OB: {lux_ob.get("score", 50)}/100 | Lux {lux_ob.get("lux_score", 50)}/100 | OB {lux_ob.get("ob_score", 50)}/100
Fusion: {fusion.get("score", 50)}/100
SP2L: {sp2l_status}
Kill Zone: {kz_name}

10C Status: {opening_range.get("status", "N/A")}
PD Liquidity: {prev_day_liquidity.get("status", "N/A")}
PDH: {p(prev_day_liquidity.get("pdh"))}
PD50: {p(prev_day_liquidity.get("pd50"))}
PDL: {p(prev_day_liquidity.get("pdl"))}
PD Targets: {p(prev_day_liquidity.get("tp1"))} / {p(prev_day_liquidity.get("tp2"))}
...
QuantumGold Adaptive Scalping V4.4 BB+STOCH FUSION"""
...
async def main():
    ...
    print("FAST SCALP MODE: ENABLED | M1 trigger + M5 primary context | RELAXED FAST SCALP | anti-over-filter")
    ...
    print(
        "Previous-Day Liquidity Hunt: "
        f"{'ENABLED' if PREV_DAY_LIQUIDITY_ENABLED else 'DISABLED'} | "
        "1D PDH/PDL/50% + 15M sweep/reclaim | Soft confirmation"
    )
    ...
    print(
        ...
        f"{'ENABLED' if BB_STOCH_ENABLED else 'DISABLED'} | "
        "Soft confirmation only"
    )
    print(
        "Premium Lux + Volumetric Order Blocks: "
        f"{'ENABLED' if LUX_OB_FUSION_ENABLED else 'DISABLED'} | Soft confirmation | Min Score={LUX_FUSION_MIN_SCORE}"
    )
    print(
        "Strategy Fusion: "
        f"{'ENABLED' if FUSION_ENABLED else 'DISABLED'} | "
        f"Min Score={FUSION_MIN_SCORE}"
    )
    print(
