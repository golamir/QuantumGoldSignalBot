...
def analyze_market(
    ...
    try:
        ...
        if signal == "🟢 BUY":
            ...
            stop_loss = (
                price
                - atr_for_trade * sl_mult
            )

            tp1 = (
                price
                + atr_for_trade * tp1_mult
            )

            tp2 = (
                price
                + atr_for_trade * tp2_mult
            )

            tp3 = (
                price
                + atr_for_trade * tp_mult
            )
...
SP2L Spike/Pullback: {sp2l_status}

Regime: {regime['name']}
10C Range: {format_price(opening_range.get("range_low"), symbol) if opening_range.get("range_low") else "N/A"} - {format_price(opening_range.get("range_high"), symbol) if opening_range.get("range_high") else "N/A"}
