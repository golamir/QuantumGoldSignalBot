...
def master_quality_filter(
    ...
    """Final V4 gate. Entry A is standard. Entry B may pass only with
    exceptionally strong confirmations. Entry C is always rejected.
    Crypto uses a softer volume rule because Yahoo crypto volume can be
    unreliable/near-zero on some intervals.
    """
    ...
    # Entry C is never allowed.
    if entry_quality == "C":
        return False, "Entry Quality C"

    is_crypto = symbol in {x[0] for x in CRYPTO_MARKETS}
    min_adx = 20 if is_crypto else MIN_ADX

    if adx_value < min_adx:
        return False, f"ADX below {min_adx}"

    # Strong Entry-B exception: it must be an unusually strong setup.
    # This prevents B from becoming a normal bypass of the MASTER filter.
    if entry_quality == "B":
        confirmation_count = sum([
            bool(structure_confirmed),
            bool(liquidity_confirmed),
            bool(fvg_confirmed),
            bool(displacement_confirmed),
        ])
        gainz_confirmed = bool(gainz_v2_confirmed or gainz_pro_confirmed)

        strong_b = (
            ai_score >= 90
            and quality_score >= 90
            and micro_confirmed
            and micro_score >= MIN_MICRO_SCORE
            and confirmation_count >= 2
            and (gainz_confirmed or confirmation_count >= 3)
        )

        if not strong_b:
            return False, "Entry Quality B - strong confirmation required"

    # Gold/Forex: volume remains a hard filter.
    # Crypto: volume is soft; strong microstructure substitutes for a bad
    # Yahoo volume ratio, while all other quality gates remain hard.
    if not volume_confirmed:
        if not (is_crypto and micro_confirmed and micro_score >= MIN_MICRO_SCORE):
    ...
    return True, "ALL V4 MASTER FILTERS PASSED"
...
def analyze_market(
    ...
    try:
        ...
        passed, reason = master_quality_filter(
            ...
            gainz_v2_confirmed=gainz_v2_confirmed,
            gainz_pro_confirmed=gainz_pro_confirmed
        )
