...
# ============================================================
# MASTER V3 QUALITY / SAFETY FILTER
# ============================================================
...
def master_quality_filter(
    ...
    # A/B entries use the normal safety gates.
    # Entry C is allowed only with stronger AI/quality support and a
    # slightly lower adaptive ADX floor; this prevents weak C setups
    # from becoming ordinary signals.
    if entry_quality == "C":
        if ai_score < 90:
            return (
                False,
                "Entry C requires AI Score >= 90"
            )

        if quality_score < 75:
            return (
                False,
                "Entry C requires Quality >= 75"
            )

        if adx_value < 23.5:
            return (
                False,
                "Entry C adaptive ADX below 23.5"
            )

    elif entry_quality not in ("A", "B"):
        return (
            False,
            f"Entry Quality {entry_quality}"
        )

    if entry_quality in ("A", "B") and adx_value < MIN_ADX:
        return (
            False,
            f"ADX below {MIN_ADX}"
        )

    # Volume is a soft confirmation/bonus, not a hard rejection.
    # It still contributes to calculate_quality_score().
    ...
    return (
        True,
        "ALL MASTER V3 QUALITY / SAFETY FILTERS PASSED | Volume="
        + ("confirmed" if volume_confirmed else "soft")
    )
...
def analyze_market(
    ...
    try:
        ...
        # ====================================================
        # TREND ALIGNMENT (V4.1)
        # ====================================================
        # M15 + H1 define the primary direction. M5 EMA/MACD
        # can temporarily disagree during a pullback, so they
        # remain quality/bonus confirmations instead of causing
        # a hard rejection.
        ...
        # Balanced AI score: Smart Score is the primary signal engine,
        # while Quality Score remains an important confirmation layer.
        # Using min(smart, quality) made strong setups disappear whenever
        # one secondary confirmation was missing.
        final_ai_score = max(
            0,
            min(
                100,
